...
def _cypher_literal(v: Any) -> str:
    ...
    if v is None:
        ...
        inner = ", ".join(f"{k}: {_cypher_literal(vv)}" for k, vv in v.items())
        return "{" + inner + "}"
...
def _props_map(d: dict) -> str:
    ...
    parts = [f"{k}: {_cypher_literal(v)}" for k, v in d.items()]
    return "{" + ", ".join(parts) + "}"
...
class GraphMemory:
    ...
    async def add_node(self, label: str, properties: dict) -> int:
        ...
        async with self._pool.acquire() as conn:
            ...
            rows = await conn.fetch(
                f"SELECT result FROM ag_catalog.cypher('memory', $$CREATE (n:{label} {props_lit}) RETURN id(n)$$) AS (result ag_catalog.agtype)"
            )
    ...
    async def merge_node(
        ...
        async with self._pool.acquire() as conn:
            ...
            await self._ensure_graph(conn)
            rows = await conn.fetch(
                f"SELECT nid FROM ag_catalog.cypher('memory', $$MERGE (n:{label} {{{match_key}: {match_lit}}}) SET n += {props_lit} RETURN id(n)$$) AS (nid ag_catalog.agtype)"
            )
    ...
    async def add_edge(
        ...
        async with self._pool.acquire() as conn:
            ...
            await self._ensure_graph(conn)
            await conn.execute(
                f"SELECT * FROM ag_catalog.cypher('memory', $$MATCH (a), (b) WHERE id(a) = {from_id} AND id(b) = {to_id} CREATE (a)-[r:{relation} {props_lit}]->(b) RETURN r$$) AS (r ag_catalog.agtype)"
            )
    ...
    async def get_nodes_by_label(self, label: str) -> list[dict]:
        ...
        async with self._pool.acquire() as conn:
            ...
            await self._ensure_graph(conn)
            rows = await conn.fetch(
                f"SELECT props FROM ag_catalog.cypher('memory', $$MATCH (n:{label}) RETURN properties(n)$$) AS (props ag_catalog.agtype)"
            )
    ...
    async def get_subgraph(self, node_id: int, hops: int = 2) -> dict:
        """Return properties of all nodes within N hops of the given node."""
        async with self._pool.acquire() as conn:
            await self._setup_connection(conn)
            await self._ensure_graph(conn)
            rows = await conn.fetch(
                f"SELECT n FROM ag_catalog.cypher('memory', $$MATCH (start)-[*1..{hops}]-(n) WHERE id(start) = {node_id} RETURN DISTINCT properties(n)$$) AS (n ag_catalog.agtype)"
            )
